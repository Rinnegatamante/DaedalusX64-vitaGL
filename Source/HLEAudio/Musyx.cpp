                                ...
                                uint8_t skip_samples)
{
    ...
    for (i = 0; i < count; ++i) {
        ...
        sceClibMemcpy(dst, frame, 2 * sizeof(frame[0]));
                      ...
                      musyx_t *musyx, uint32_t sfx_ptr, uint16_t idx)
{
    ...
    sceClibMemcpy(buffer, musyx->subframe_740_last4, 4 * sizeof(int16_t));
    sceClibMemcpy(musyx->subframe_740_last4, subframe + SUBFRAME_SIZE - 4, 4 * sizeof(int16_t));
