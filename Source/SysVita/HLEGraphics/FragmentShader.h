...
"float4 texelFetch(sampler2D tex, int2 size, int2 coord)\n"
"{\n"
"    return tex2D(tex, float2(float(coord.x) / float(size.x),\n"
...
"{\n"
"	int2 coord_clamped = clamp((float2)coord, (float2)(tile_tl<<3), (float2)(tile_br<<3));\n"
...
"	int2 br = tile_br<<3;\n"
"\n"
"	int2 coord_clamped = clamp((float2)coord, (float2)tl, (float2)br);\n"
"	int2 coord_out     = imix(coord, coord_clamped, clamp_enable);\n"
"\n"
...
"	uv0 = clampBilinear(uv0, tile_tl, tile_br, clamp_enable, /*out */fracb);\n"
"\n"
...
"\n"
"	return bilinear(col_00, col_01, col_10, col_11, fracb);\n"
"}\n"
"\n"
...
"	int2 uv0 = int2(st_in);\n"
"	uv0 = shift(uv0, shift_scale);\n"
"	uv0 = clampBilinear(uv0, tile_tl, tile_br, clamp_enable, /*out */fracc);\n"
"\n"
"	int2 uv1 = uv0 + int2(1,1);\n"
"\n"
"	uv0 = mask(uv0, mirror_bits, mask_bits);\n"
...
"	fracc = imix(fracc, fracc & bilerp_wrap_enable, lessThan(uv1, uv0));\n"
"\n"
"	float4 col_00  = texelFetch(tex, int2(uv0.x, uv0.y), 0);\n"
"	float4 col_01  = texelFetch(tex, int2(uv0.x, uv1.y), 0);\n"
"	float4 col_10  = texelFetch(tex, int2(uv1.x, uv0.y), 0);\n"
"	float4 col_11  = texelFetch(tex, int2(uv1.x, uv1.y), 0);\n"
"\n"
"	return bilinear(col_00, col_01, col_10, col_11, fracc);\n"
"}\n"
"\n"
